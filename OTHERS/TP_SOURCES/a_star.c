...
double A_star(grid G, heuristic h){
  ...
  // Par exemple, dès vous ajoutez un sommet à P ou à Q mais aussi
  // lorsque vous reconstruisez le chemin à la fin de la fonction
  // (marquer les sommets à MK_PATH). Lorsqu'un sommet passe dans Q
  // vous pourrez le marquer à MK_FRONT (dans son champs .mark) pour
  // le distinguer à l'affichage des sommets de P (couleur
  // différente). Cependant, prennez garde de ne le faire que si le
  // sommet n'est pas marqué, c'est-à-dire que si sa marque est
  // MK_NULL, sous peine de visiter à l'infini les mêmes sommets à
  // cause de la gestion paresseuse de Q.
